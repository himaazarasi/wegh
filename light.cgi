...
#┌─────────────────────────────────
#│ LightBoard : light.cgi - 2022/03/10
...
sub find_data {
	...
	# 検索実行
	my ($hit,@log) = search($in{word},$in{cond},"$cf{datadir}/log.cgi") if ($in{word} ne '');
...
sub past_page {
	# 過去ログ番号
	open(IN,"$cf{datadir}/pastno.dat") or error("open err: pastno.dat");
	my $pastnum = <IN>;
	close(IN);
	...
	# 対象ログ定義
	my $file = "$cf{datadir}/past/" . sprintf("%04d", $in{pno}) . ".cgi";
...
sub save_tmp {
	...
	# 採番
	open(NO,"+< $cf{datadir}/num.dat") or error("open err: num.dat");
	eval "flock(NO,2);";
	my $num = <NO>;
	
	# 前ログ
	if (-e "$cf{datadir}/log/$num.cgi") {
	...
	# ファイル生成
	open(TMP,"+> $cf{datadir}/log/$num.cgi") or error("write err: $num.cgi");
	eval "flock(TMP,2);";
	print TMP "$date<>$in{name}<>$in{email}<>$in{sub}<>$in{comment}<>$in{url}<>$host<>$pwd<>$time<>";
	close(TMP);
	
	chmod(0666,"$cf{datadir}/log/$num.cgi");
}
